module.exports = {
  description: "Toggle the No Embed-role.",
  usage: {
...
module.exports.run = async (client, message, args) => {
  ...
  for (const member of members) {
    let success;
    if (member.roles.cache.get(constants.roles.noembed)) {
      diff[member.user.tag] = "-";
      success = await member.roles.remove(constants.roles.noembed).catch(() => null)
    } else {
      diff[member.user.tag] = "+";
      success = await member.roles.add(constants.roles.noembed).catch(() => null)
    }

    if (!success) diff[member.user.tag] = "?";
  }

  message.channel.send(`${constants.emojis.tickyes} Members blocked from embeds has been changed: \`\`\`diff\n${Object.keys(diff).map(tag => diff[tag] + " " + tag).join("\n")}\`\`\``)
}