...
module.exports.run = async ({ client, member, respond }, { subserver: acronym }) => {
  ...
  oauth.tokenRequest({
    ...
    if (subMember) return respond(`${emojis.tickno} You are already in this subserver.`);
    if (!access) return respond(`${emojis.tickno} You do not have access to add yourself to this subserver.`);
    ...
    oauth.addMember({
      ...
      respond(`${emojis.tickyes} You are now added to **${subserver.name}**.`);
    }).catch(e => {
      console.log(e);
      respond(`${emojis.tickno} An unknown error occurred when trying to add you to the subserver.`);
    });
  }).catch(e => {
    console.log(e);
    respond(`${emojis.tickno} Your OAuth2 is not working! \`/oauth\``);
  });
};